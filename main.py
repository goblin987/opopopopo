    ...
    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if query and query.data:
            ...
            KNOWN_HANDLERS = {
                ...
                "adm_emergency_mode": admin.handle_adm_emergency_mode,
                "adm_toggle_green_emoji": admin.handle_adm_toggle_green_emoji,
                "adm_toggle_eur_symbol": admin.handle_adm_toggle_eur_symbol,
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    STATE_HANDLERS = {
        # User Handlers (from user.py)
        'awaiting_captcha': user.handle_captcha_answer,
...
# Telegram Server IP Ranges (updated as of 2024)
TELEGRAM_IP_RANGES = [
    '149.154.160.0/20',
    '91.108.4.0/22',
    '91.108.8.0/22',
...
async def telegram_webhook():
    ...
    # IP Whitelist Check - Only allow Telegram servers
    client_ip = request.headers.get('X-Forwarded-For', request.remote_addr)
    if client_ip:
        # Get the first IP if there's a chain
        client_ip = client_ip.split(',')[0].strip()
    
    if not is_telegram_ip(client_ip):
        logger.warning(f"🚫 Rejected webhook from non-Telegram IP: {client_ip}")
        return Response("Unauthorized", status=403)
    
    logger.debug(f"✅ Accepted webhook from Telegram IP: {client_ip}")
