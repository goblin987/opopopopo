...
def toggle_emergency_green_emoji():
    """Toggle hiding of green emoji (🟢) in products."""
    conn = None
    try:
        conn = get_db_connection()
        c = conn.cursor()
        c.execute("UPDATE emergency_settings SET hide_green_emoji = NOT hide_green_emoji WHERE id = 1")
        conn.commit()
        c.execute("SELECT hide_green_emoji FROM emergency_settings WHERE id = 1")
        result = c.fetchone()
        new_state = bool(result['hide_green_emoji']) if result else False
        logger.info(f"Emergency mode: Green emoji hiding toggled to {new_state}")
        return new_state
    except sqlite3.Error as e:
        logger.error(f"Error toggling green emoji emergency mode: {e}")
        return False
    finally:
        if conn:
            conn.close()
...
def toggle_emergency_eur_symbol():
    """Toggle hiding of EUR symbols in prices."""
    conn = None
    try:
        conn = get_db_connection()
        c = conn.cursor()
        c.execute("UPDATE emergency_settings SET hide_eur_symbol = NOT hide_eur_symbol WHERE id = 1")
        conn.commit()
        c.execute("SELECT hide_eur_symbol FROM emergency_settings WHERE id = 1")
        result = c.fetchone()
        new_state = bool(result['hide_eur_symbol']) if result else False
        logger.info(f"Emergency mode: EUR symbol hiding toggled to {new_state}")
        return new_state
    except sqlite3.Error as e:
...
def filter_green_emoji_from_text(text: str) -> str:
    """Remove green emoji (🟢) from text if emergency mode is active."""
    emergency_settings = get_emergency_settings()
    if emergency_settings.get('hide_green_emoji', False):
        # Remove 🟢 emoji from text
        return text.replace('🟢', '').strip()
    return text


def filter_size_gram_symbol(size: str) -> str:
    """Remove 'g' from size (e.g., '2g' becomes '2') if emergency mode is active."""
    emergency_settings = get_emergency_settings()
    if emergency_settings.get('hide_green_emoji', False):
        # Remove 'g' from size string
        return size.replace('g', '').strip()
    return size
...
def is_user_verified(user_id: int) -> bool:
    """Check if user has passed CAPTCHA verification."""
    conn = None
    try:
        conn = get_db_connection()
        c = conn.cursor()
        c.execute("SELECT is_verified FROM captcha_verification WHERE user_id = ?", (user_id,))
        result = c.fetchone()
        return bool(result['is_verified']) if result else False
    except sqlite3.Error as e:
        logger.error(f"Error checking CAPTCHA verification for user {user_id}: {e}")
        return False
    finally:
        if conn:
            conn.close()
...
def create_captcha_challenge(user_id: int) -> str:
    """Create a new CAPTCHA challenge for a user."""
    conn = None
    try:
        conn = get_db_connection()
        c = conn.cursor()
        ...
        c.execute("""
            INSERT INTO captcha_verification (user_id, is_verified, captcha_code, captcha_attempts, last_attempt_time)
            VALUES (?, 0, ?, 0, ?)
            ON CONFLICT(user_id) DO UPDATE SET 
                captcha_code = excluded.captcha_code,
                captcha_attempts = 0,
                last_attempt_time = excluded.last_attempt_time
        """, (user_id, captcha_code, current_time))
        
        conn.commit()
        logger.info(f"Created CAPTCHA challenge for user {user_id}")
        return captcha_code
        
    except sqlite3.Error as e:
        logger.error(f"Error creating CAPTCHA challenge for user {user_id}: {e}")
        return generate_captcha_code()  # Return a code anyway
    finally:
        if conn:
            conn.close()


def verify_captcha_answer(user_id: int, user_answer: str) -> tuple[bool, str]:
    """
    ...
    try:
        ...
        c.execute("SELECT captcha_code, captcha_attempts FROM captcha_verification WHERE user_id = ?", (user_id,))
        result = c.fetchone()
        
        if not result:
            return False, "No CAPTCHA challenge found. Please restart /start"
        
        correct_code = result['captcha_code']
        attempts = result['captcha_attempts']
        
        # Check if too many attempts (max 3)
        if attempts >= 3:
            return False, "Too many failed attempts. Please restart /start"
        
        # Verify answer (case-insensitive)
        if user_answer.upper().strip() == correct_code.upper():
            # Mark as verified
            ...
            c.execute("""
                ...
                WHERE user_id = ?
            """, (verified_time, user_id))
            conn.commit()
            logger.info(f"✅ User {user_id} passed CAPTCHA verification")
...
def reset_captcha_verification(user_id: int):
    """Reset CAPTCHA verification for a user (for testing or admin override)."""
    conn = None
    try:
        conn = get_db_connection()
        c = conn.cursor()
        c.execute("DELETE FROM captcha_verification WHERE user_id = ?", (user_id,))
        conn.commit()
        logger.info(f"Reset CAPTCHA verification for user {user_id}")
    except sqlite3.Error as e:
        logger.error(f"Error resetting CAPTCHA for user {user_id}: {e}")
    finally:
        if conn:
            conn.close()
...
def format_currency(value, include_eur_symbol=True):
    """Format currency value, optionally hiding EUR symbol based on emergency mode."""
    try: 
        formatted = f"{Decimal(str(value)):.2f}"
        if include_eur_symbol:
            # Check if EUR symbol should be hidden in emergency mode
            emergency_settings = get_emergency_settings()
            if not emergency_settings.get('hide_eur_symbol', False):
                formatted += " EUR"
        return formatted
    except (ValueError, TypeError): 
        logger.warning(f"Could not format currency {value}")
        return "0.00"

def format_discount_value(dtype, value):
    try:
        if dtype == 'percentage': return f"{Decimal(str(value)):.1f}%"
        elif dtype == 'fixed': return f"{format_currency(value)}"
        return str(value)
...
def get_failed_payments_for_recovery():
    ...
    try:
        ...
        c.execute("""
            SELECT payment_id, user_id, target_eur_amount, currency, expected_crypto_amount,
                   basket_snapshot_json, discount_code_used, created_at
            FROM pending_deposits 
