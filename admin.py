...
async def handle_admin_menu(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🚨 Emergency Mode", callback_data="adm_emergency_mode")],
...
async def handle_payment_recovery_id_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # CRITICAL: Verify payment was actually received by NOWPayments
        logger.info(f"Verifying payment {payment_id} with NOWPayments before recovery...")
        payment_verification = await payment.verify_payment_with_nowpayments(payment_id)
        
        if not payment_verification.get('verified'):
            error_reason = payment_verification.get('reason', 'unknown')
            await send_message_with_retry(context.bot, chat_id, 
                f"❌ Payment {payment_id} was NOT received by NOWPayments. Reason: {error_reason}\n\n"
                f"⚠️ Cannot recover payment that was never received. Check NOWPayments dashboard.", 
                parse_mode=None)
            return
        
        # ADDITIONAL SECURITY: Verify payment amount matches expected amount
        actually_paid = payment_verification.get('actually_paid')
        if not actually_paid or actually_paid <= 0:
            await send_message_with_retry(context.bot, chat_id, 
                f"❌ Payment {payment_id} verification failed: No valid payment amount received.\n\n"
                f"⚠️ Cannot recover payment with zero or invalid amount.", 
                parse_mode=None)
            return
        
        logger.info(f"Payment {payment_id} verified with NOWPayments. Status: {payment_verification.get('status')}, Amount: {actually_paid}")
    ...
    try:
        if decision == '1':  # Proceed anyway
            ...
            await send_message_with_retry(context.bot, chat_id, 
                ...
                f"⚠️ Manual refund required through NOWPayments dashboard or crypto wallet.", 
                parse_mode=None)
            
            # Remove pending deposit
            await asyncio.to_thread(remove_pending_deposit, payment_id, trigger="manual_recovery_refund")
            
            # TODO: Implement automatic refund if possible with NOWPayments API
...
async def handle_adm_emergency_mode(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    """Display emergency mode control panel."""
    query = update.callback_query
    user_id = query.from_user.id
    
    if not is_primary_admin(user_id):
        return await query.answer("Access denied. Primary admin only.", show_alert=True)
...
async def handle_adm_toggle_green_emoji(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    """Toggle hiding of green emoji products."""
    query = update.callback_query
    user_id = query.from_user.id
    ...
    # Toggle the setting
    from utils import toggle_emergency_green_emoji, log_admin_action
    new_state = await asyncio.to_thread(toggle_emergency_green_emoji)
    
    # Log the action
    action_type = "EMERGENCY_HIDE_GREEN_EMOJI" if new_state else "EMERGENCY_SHOW_GREEN_EMOJI"
    log_admin_action(
        admin_id=user_id,
        action=action_type,
        reason="Emergency mode toggle for mass reporting protection"
    )
    
    await query.answer(
        f"✅ Green emoji products are now {'HIDDEN' if new_state else 'VISIBLE'}",
        show_alert=True
    )
    
    # Refresh the emergency mode menu
    await handle_adm_emergency_mode(update, context, params)


async def handle_adm_toggle_eur_symbol(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    """Toggle hiding of EUR symbols."""
    query = update.callback_query
    user_id = query.from_user.id
    ...
    # Toggle the setting
    from utils import toggle_emergency_eur_symbol, log_admin_action
    new_state = await asyncio.to_thread(toggle_emergency_eur_symbol)
    
    # Log the action
    action_type = "EMERGENCY_HIDE_EUR_SYMBOL" if new_state else "EMERGENCY_SHOW_EUR_SYMBOL"
    log_admin_action(
        admin_id=user_id,
        action=action_type,
        reason="Emergency mode toggle for mass reporting protection"
    ...
    await query.answer(
        f"✅ EUR symbols are now {'HIDDEN' if new_state else 'VISIBLE'}",
        show_alert=True
    )
