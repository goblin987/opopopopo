...
SUPPORTED_CRYPTO = {
    ...
    'eth': 'ETH',
    'sol': 'SOL',
    'usdttrc20': 'USDT (TRC20)', # Example for TRC20 USDT
    'usdterc20': 'USDT (ERC20)', # Example for ERC20 USDT
    'usdtbsc': 'USDT (BEP20)',   # Example for BEP20 USDT (Binance Smart Chain)
    'usdtsol': 'USDT (SOL)',     # Example for Solana USDT
    'usdctrc20': 'USDC (TRC20)', # Example for TRC20 USDC
    'usdcerc20': 'USDC (ERC20)', # Example for ERC20 USDC
    'usdcsol': 'USDC (SOL)',     # Example for Solana USDC
    'ton': 'TON',
    # Add other supported currencies/networks here as needed
...
async def handle_captcha_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    from utils import verify_captcha_answer
    
    success, message = await asyncio.to_thread(verify_captcha_answer, user_id, user_answer)
    
    if success:
        # CAPTCHA passed, clear state and proceed to start
        context.user_data.pop('state', None)
        await send_message_with_retry(context.bot, update.effective_chat.id, message, parse_mode=None)
        
        # Show the main menu now
        await start(update, context)
    else:
        # CAPTCHA failed
        await send_message_with_retry(context.bot, update.effective_chat.id, message, parse_mode=None)
        
        # If too many attempts, stay in state (user must restart /start)
        if "restart" in message.lower():
            context.user_data.pop('state', None)
            await send_message_with_retry(context.bot, update.effective_chat.id, 
                "Please use /start to try again.", parse_mode=None)
    ...
    try:
        ...
        if not products:
            ...
            for row in products:
                ...
                # Filter size display if emergency mode is active
